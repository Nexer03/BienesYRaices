...
<div class="container py-4">
  ...
  <h3 class="mb-3">Editar visita</h3>
  ...
  <form method="POST" action="{{ route('agent.visits.update', $visit) }}" class="card">
    @csrf @method('PUT')
    <div class="card-body">
      ...
      <div class="mb-3">
        ...
        <select name="property_id" class="form-select" required>
          @foreach($properties as $id => $title)
            <option value="{{ $id }}" @selected(old('property_id', $visit->property_id)==$id)>{{ $title }}</option>
          @endforeach
        </select>
      ...
      <div class="mb-3">
        ...
        <select name="client_id" class="form-select" required>
          @foreach($clients as $id => $name)
            <option value="{{ $id }}" @selected(old('client_id', $visit->client_id)==$id)>{{ $name }}</option>
          @endforeach
        </select>
      ...
      <div class="mb-3">
        ...
        <input type="datetime-local" name="visit_date" class="form-control"
               value="{{ old('visit_date', optional($visit->visit_date)->format('Y-m-d\TH:i')) }}" required>
